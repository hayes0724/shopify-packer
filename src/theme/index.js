...
module.exports.create = async (args) => {
    ...
    spinner.succeed(
        `Theme ${chalk.cyan(result.theme.name)} with id ${chalk.cyan(result.theme.id)} created and config.json updated!`
    );
    console.log(table(parsed));
};
...
module.exports.remove = async (args) => {
    clearConsole();
    const spinner = ora(chalk.magenta(`'Deleting theme for environment: ${Environment.getEnvNameValue()} ...'`));
    spinner.start();
    ...
    if (result.status === 'error') {
        ...
        manager.setId("");
        spinner.succeed(`Theme ${chalk.cyan(result.name)} with id ${chalk.cyan(result.id)} was deleted`);
    }
};
...
const _parseThemes = (themes) => {
    ...
    for (let [key, value] of Object.entries(themes[0])) {
