...
module.exports.create = (args) => {
    ...
    return axios.post('/admin/themes.json', data)
        .then((res) => {
            return res.data;
        })
        .catch((error) => {
...
module.exports.remove = async (args) => {
    ...
    return axios.delete(`/admin/themes/${themeId}.json`)
        .then((res) => {
            return res.data;
        })
        .catch((error) => {
...
const list = () => {
    _headers();
    return axios.get('/admin/themes.json')
        .then((res) => {
...
const _headers = () => {
    axios.defaults.headers.common['X-Shopify-Access-Token'] = Environment.getPasswordValue();
...
const _isMainTheme = async (themeId) => {
    ...
    const match = themes.filter((theme) => {return theme.id === parseInt(themeId)});
    if (match.length < 1) {
        return {
            status: 'error',
