...
module.exports = class Environment {
  constructor(env) {
    this.config = require(paths.settings);
    this.env = env ? env : 'development';
    this.settings = this.config.themes[this.env];
  ...
  _setEnvironment() {
    ...
    if (this.env !== 'development' && this.env) {
      if (!this.config.themes[this.env].id) {
        console.log(
          chalk.red(`${this.env} is not a valid theme name in config.json`)
        );
        process.exit(1);
      }
    }
    ...
    process.env.NODE_ENV = process.env.NODE_ENV
      ? process.env.NODE_ENV
      : this.env;
    process.env.THEME_ID = process.env.THEME_ID
      ? process.env.THEME_ID
      : this.settings.id;
    process.env.PASSWORD = process.env.PASSWORD
      ? process.env.PASSWORD
      : this.settings.password;
    process.env.STORE = process.env.STORE
      ? process.env.STORE
      : this.settings.store;
    process.env.IGNORE_FILES = process.env.IGNORE_FILES
      ? process.env.IGNORE_FILES
      : this.settings.ignore.join(':');
  }
  ...
  static _validateStore() {
    ...
    if (store.length === 0) {
      errors.push(
        new Error(
          `${Environment.getEnvNameValue()} - store url must not be empty`
        )
      );
    } else if (
  ...
  static _validatePassword() {
    ...
    if (password.length === 0) {
      errors.push(
        new Error(
          `${Environment.getEnvNameValue()} - password must not be empty`
        )
      );
    } else if (!/^\w+$/.test(password)) {
      errors.push(
        new Error(
          `${Environment.getEnvNameValue()} can only contain numbers and letters`
        )
      );
    }
